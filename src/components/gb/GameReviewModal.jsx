...
export default function GameReviewModal({ isOpen, onClose, game, onImport, onSavedReview = null, onStatusChange = null }) {
  ...
  useEffect(() => {
    if (!isOpen) {
      setLoading(false); setReview(null); setError(null);
      setEditing(false); setDraftRating(0); setDraftText(""); setDraftIsPublic(true); setSaving(false);
      setImporting(false); setImportError(null); setAutoSaving(false); setAutoSaveSuccess(false); setStatus(null);
      if (autoSaveRef.current) { clearTimeout(autoSaveRef.current); autoSaveRef.current = null; }
    }
  }, [isOpen]);

  useEffect(() => { if (isOpen && contentRef.current) contentRef.current.scrollTop = 0; }, [game, isOpen]);
  ...
  const gb = game?.giantbomb ? game.giantbomb : (game || {});
  ...
  const date = rawDate ? (() => { try { return new Date(rawDate).toLocaleDateString(); } catch { return rawDate; } })() : "Data não disponível";

  const platformsStr = ((gb.platforms || game?.platforms) || []).map?.(p => p.name).filter(Boolean).join(", ") || "—";
  const publishersStr = ((gb.publishers || game?.publishers) || []).map?.(p => p.name).filter(Boolean).join(", ") || "—";
  const genresStr = ((gb.genres || game?.genres) || []).map?.(g => g.name).filter(Boolean).join(", ") || "—";
  ...
  useEffect(() => {
    ...
    let mounted = true;
    setLoading(true); setError(null); setReview(null); setEditing(false);
    setDraftRating(0); setDraftText(""); setDraftIsPublic(true);

    (async () => {
      try {
        const base = getApiBase();
        const token = localStorage.getItem("token");
        const params = game.id ? `?game_id=${encodeURIComponent(game.id)}` : ( (game.external_guid || game.externalGuid || game.external_id || gb.guid || gb.id) ? `?external_guid=${encodeURIComponent(game.external_guid || game.externalGuid || game.external_id || gb.guid || gb.id)}` : "" );
        const res = await fetch(`${base}/reviews/me${params}`, { headers: { "Content-Type":"application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) }});
        if (!mounted) return;
        if (!res.ok) {
          if (res.status === 404) { setReview(null); setLoading(false); return; }
          throw new Error(`status ${res.status}`);
        }
        const data = await res.json();
        const normalized = {
          ...data,
          rating: typeof data.rating !== "undefined" ? Number(data.rating) : (data.review_text ? 0 : null),
  ...
  async function saveReview(overrides = {}) {
    ...
    const normalized = {
      ...res,
      rating: typeof res.rating !== "undefined" ? Number(res.rating) : (res.review_text ? 0 : null),
      review_text: res.review_text ?? res.text ?? res.body ?? null,
      is_public: typeof res.is_public === "boolean" ? res.is_public : (res.is_public ?? true),
    };
    setReview(normalized);
    setDraftText(normalized.review_text || "");
    setDraftRating(Math.round(Number(normalized.rating ?? 0)));
    setDraftIsPublic(normalized.is_public ?? true);
    try { typeof onStatusChange === "function" && onStatusChange({ ...game, id: game?.id, external_guid: game?.external_guid ?? game?.externalGuid ?? null, rating: normalized.rating, review: normalized }); } catch(e){ console.warn(e); }
    try { typeof onSavedReview === "function" && onSavedReview(normalized, creating); } catch(e){ console.warn(e); }
    return normalized;
  }
  ...
  async function handleSaveReview() {
    ...
    try {
      ...
      setReview(saved); setEditing(false); toast.success("Review salva");
    } catch (err) {
      console.error(err); setError(err.message || String(err)); toast.error("Erro ao salvar review");
    } finally { setSaving(false); }
  }

  function scheduleAutoSave(newRating) {
    if (!game) return;
    if (autoSaveRef.current) clearTimeout(autoSaveRef.current);
    setAutoSaveSuccess(false);
    autoSaveRef.current = setTimeout(async () => {
      setAutoSaving(true); setAutoSaveSuccess(false);
      try {
        const saved = await saveReview({ rating: newRating, text: draftText || (review?.review_text || "") });
        setReview(saved);
        setAutoSaveSuccess(true);
        try { typeof onStatusChange === "function" && onStatusChange({ ...game, id: game?.id, external_guid: game?.external_guid ?? game?.externalGuid ?? null, rating: saved.rating, review: saved }); } catch(e){ console.warn(e); }
        setTimeout(() => setAutoSaveSuccess(false), 1500);
      } catch (err) {
        console.error("autosave", err);
        setError(err.message || String(err)); toast.error("Erro ao salvar review");
      } finally { setAutoSaving(false); }
    }, 800);
  }
  function handleStarsChange(newVal) { const v = Math.round(Number(newVal || 0)); setDraftRating(v); scheduleAutoSave(v); }
  ...
  async function handleImportClick() {
    ...
    try {
      ...
      console.error("import", err); setImportError(err?.message || String(err));
  ...
  async function handleChangeStatus(newStatus) {
    ...
    setStatus(newStatus); setUpdatingStatusTo(newStatus);
    try {
      ...
      try { typeof onStatusChange === "function" && (saved && (saved.id || saved.game_id || saved.data) ? onStatusChange(saved) : onStatusChange({ id: game?.id ?? null, external_guid: game?.external_guid ?? null, status: savedStatus })); } catch(e){ console.warn(e); }
      toast.success(`Status atualizado: ${savedStatus}`);
    } catch (err) {
      console.error(err); setStatus(previous); toast.error("Não foi possível atualizar o status");
    } finally { setUpdatingStatusTo(null); }
  }

  const entry = { initial: { opacity:0, y:8, scale:0.996 }, animate: { opacity:1, y:0, scale:1, transition:{ type:"spring", stiffness:240, damping:20 } } };

  const displayRating = (typeof review?.rating !== "undefined") ? Math.round(Number(review.rating)) : (typeof gb.rating !== "undefined" ? Math.round(Number(gb.rating)) : 0);
  const reviewRating = review ? Math.round(Number(review.rating || 0)) : null;
  const effectiveRating = editing ? draftRating : (typeof reviewRating === "number" && reviewRating !== null ? (Number(draftRating) !== Number(reviewRating) ? draftRating : reviewRating) : (Number(draftRating) !== 0 ? draftRating : displayRating));
              ...
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-[auto_1fr] gap-6 items-start">
                <div className="flex-shrink-0">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 items-center">
                    <RatingStars value={effectiveRating} onChange={handleStarsChange} readOnly={false} max={10} size={18} showTooltip />
