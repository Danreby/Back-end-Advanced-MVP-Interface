...
/**
 * Navbar para GameCatalog com toggle Dark/Light mode.
 * - Persiste preferência em localStorage ("theme" = "dark" | "light")
 * - Aplica/remova class "dark" em document.documentElement (Tailwind class strategy)
 * - Props:
 *    - user: objeto user (ex: { email: "..." })
 *    - onLogout: função chamada ao clicar em Sair
 *    - onNavigate (opcional): função para navegação client-side (ex: router.push)
 */
export function Navbar({ user, onLogout, onNavigate }) {
  ...
  useEffect(() => {
    try {
      ...
      // se localStorage não estiver disponível, mantemos light por padrão
      setTheme("light");
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            >
                {/* Sun */}
                <svg xmlns="http://www.w3.org/2000/svg" className={`absolute w-5 h-5 text-yellow-500 transition-all duration-500 transform ${theme === "dark" ? "opacity-0 scale-75 rotate-90" : "opacity-100 scale-100 rotate-0"}`} viewBox="0 0 16 16" fill="currentColor">
